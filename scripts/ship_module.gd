...
var is_destroyed_state: bool = false
...
func _process(delta):
	...
	# Auto-repair when not on fire
	if current_hp < max_hp and not is_on_fire and not is_destroyed_state:
		repair_timer += delta
		if repair_timer >= 1.0:
			repair_timer = 0.0
...
func take_damage(damage: float, can_start_fire: bool = true):
	if is_destroyed_state:
		return
		
	current_hp -= damage
	print("[Module ", name, "] Took ", damage, " damage. HP: ", current_hp, "/", max_hp)
	
	# Damage flash effect
	if sprite:
		sprite.modulate = Color(1.5, 1.0, 1.0)  # Red-tinted flash
		await get_tree().create_timer(GameConstants.HIT_FLASH_DURATION).timeout
		if sprite and not is_destroyed_state:
			_update_visual_state()
...
func _update_visual_state():
	...
	if is_destroyed_state:
		# Destroyed state: dark gray with red tint
	...
	if is_focused:
		...
		# Normal/light damage: default or slight tint
		var base_color = Color(1.0, 1.0, 1.0, 1.0)
		# Keep the module's base tint if it had one (for enemy modules)
		if abs(sprite.modulate.r - 1.0) > 0.1 or abs(sprite.modulate.g - 1.0) > 0.1:
			# Preserve existing tint
			pass
		else:
			sprite.modulate = base_color

func _enter_destroyed_state():
	is_destroyed_state = true
	is_on_fire = false
